...
/**
 * One hit per word (the index has one hit per *occurrence*, so توت would
 * otherwise be picked twice as often), plus every slot the letter fills there.
 */
function pickWordHit(rng: Rng, target: string, tier: LevelDef['tier']): { hit: WordHit; slots: Slot[] } | null {
  const hits = wordsWithLetter(target).filter((h) => h.word.difficulty <= maxDiff(tier));
  const firstPerWord = hits.filter((h, i) => hits.findIndex((o) => o.word.ar === h.word.ar) === i);
  if (!firstPerWord.length) return null;
  const hit = pick(rng, firstPerWord);
  const slots = (['start', 'middle', 'end'] as Slot[])
    .filter((s) => hits.some((h) => h.word.ar === hit.word.ar && h.slot === s));
  return { hit, slots };
}
...
function qPositionDetection(rng: Rng, target: string, tier: LevelDef['tier']): Question | null {
  const found = pickWordHit(rng, target, tier);
  if (!found) return null;
  const { hit, slots } = found;
  const options: Option[] = shuffle(
    rng,
    (['start', 'middle', 'end'] as Slot[]).map((s, i) => ({
      id: `s${i}`,
      render: { kind: 'slot', slot: s } as Option['render'],
      correct: slots.includes(s),
    })),
  );
  return {
    id: nextId(),
    ...
    subPrompt: slots.length > 1 ? msg('q.multiHint') : undefined,
    options,
    need: slots.length,
    difficulty: hit.word.difficulty,
    teachCorrect: msg('fb.correctPos', { letter: target, slots: slots.join(',') }),
    value: 140,
  };
}
...
function qWordHunt(rng: Rng, target: string, tier: LevelDef['tier']): Question | null {
  ...
  return {
    ...
    need: 1,
    difficulty: hit.word.difficulty,
    teachCorrect: msg('fb.correctHunt', { letter: target, slots: slots.join(',') }),
    value: 150,
  };
}
...
export function buildThreeWords(letter: string, seed: number, tier: LevelDef['tier']): ThreeWordSlot[] {
  ...
  for (const slot of slots) {
    ...
    // A word that ALSO has the letter in this slot (توت: start + end) is not a decoy.
    const inSlot = new Set(wordsWithLetterAt(letter, slot).map((h) => h.word.ar));
    const otherSlot = wordsWithLetter(letter)
      .filter((h) => !inSlot.has(h.word.ar) && !used.has(h.word.ar))
      .map((h) => h.word);
