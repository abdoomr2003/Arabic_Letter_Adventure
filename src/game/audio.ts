...
/**
 * A gentle looping backing track, generated rather than downloaded.  It is a
 * slow arpeggio over a soft pad, quiet enough to sit behind reading Arabic.
 */
let musicTimer: number | null = null;
let musicGain: GainNode | null = null;

const SCALE = [0, 2, 3, 5, 7, 8, 10]; // a calm minor-ish mode
const ROOT = 196; // G3
...
export function startMusic() {
  ...
  musicGain.gain.linearRampToValueAtTime(0.045, c.currentTime + 2);
  ...
  beat();
  musicTimer = window.setInterval(beat, 1400);
}
...
export function stopMusic() {
  ...
  if (musicGain && c) {
    ...
    const g = musicGain;
    window.setTimeout(() => { try { g.disconnect(); } catch { /* already gone */ } }, 800);
  }
  musicGain = null;
}
